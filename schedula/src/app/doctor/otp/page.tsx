...
export default function DoctorOtpPage() {
    ...
    useEffect(() => {
        ...
        if (otp && expiry && Date.now() < expiry && pendingUser && isDoctor) {
            ...
            alert("OTP expired or invalid session! Please login again.");
    ...
    const handleVerifyOtp = () => {
        ...
        if (enteredOtp === storedOtp && pendingUser) {
            // ... (rest of verification: parse user, check type, set user, clear items, redirect) ...
             ...
             if (user.type !== 'doctor') {
                  alert("Authentication error. Not a doctor account.");
                  router.push("/doctor/login");
                  return;
             }
    ...
    if (isValidSession === null) {
        return <div className="min-h-screen flex justify-center items-center">Verifying session...</div>;
    }
                ...
                <div className="max-w-sm w-full bg-white">
                    ...
                    <div className="mt-4 text-sm text-gray-600 text-center">
                        {timer > 0 ? (
                            ...
                            <button onClick={handleResendOtp} className="text-cyan-600 font-medium hover:underline"> Resend OTP </button>
                        )}
                    </div>
                    ...
                    {otpVisible && (
                        <div className="mt-5 bg-cyan-50 border border-cyan-200 text-cyan-700 px-4 py-2 rounded-md text-sm flex justify-center items-center gap-2">
                            ...
                            <button onClick={() => navigator.clipboard.writeText(otpVisible)} className="text-xs text-cyan-600 underline hover:text-cyan-800"> Copy </button>
                        </div>
                    )}
                </div>
            </div>
