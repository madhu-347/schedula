...
const initialMockMessages = [
  { id: 1, sender: 'doctor', text: 'Hi, I sorry to hear that your not feeling well here are some immediate things you can try ..', time: '16:00' },
] as Message[];
    ...
    <main className="min-h-screen bg-gray-100 flex flex-col h-screen">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-20 w-full flex-shrink-0">
        ...
        <div className="flex items-center p-4 max-w-6xl mx-auto w-full">
          <Link href="/user/dashboard" className="p-2 -ml-2 text-gray-600 hover:text-gray-900">
            <ArrowLeft size={24} />
          </Link>
      ...
      <div className="flex-1 overflow-hidden w-full p-4 flex flex-col md:flex-row md:gap-4 max-w-6xl mx-auto">
        ...
        <div className="w-full md:w-1/2 flex-shrink-0 mb-4 md:mb-0">
          <div className="bg-white rounded-xl shadow-lg p-4 h-full sticky top-20"> {/* Made patient info sticky */}
            <h3 className="font-bold text-sm mb-3 text-gray-800">Full Name: <span className="font-medium text-gray-900">{patient.name}</span></h3>
            <div className="grid grid-cols-3 gap-4 mb-4 text-left border-b border-gray-100 pb-4">
               {/* Age/Weight/Relation */}
               <div><p className="text-xs text-gray-500 mb-1">Age</p><p className="text-md font-semibold text-gray-900">{patient.age}</p></div>
               <div><p className="text-xs text-gray-500 mb-1">Weight</p><p className="text-md font-semibold text-gray-900">{patient.weight}</p></div>
               <div><p className="text-xs text-gray-500 mb-1">Relation</p><p className="text-md font-semibold text-gray-900">{patient.relation}</p></div>
            </div>
            <h3 className="font-semibold text-sm mb-1 text-gray-800">Problem</h3>
            <p className="text-md font-medium text-gray-900 mb-3">{patient.problem}</p>
            <h3 className="font-semibold text-sm mb-1 text-gray-800">Mobile</h3>
            <p className="text-md font-medium text-cyan-600">{patient.mobile}</p>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 bg-white rounded-xl shadow-lg flex flex-col overflow-hidden h-full">
          ...
          <div className="p-3 md:p-4 border-b border-gray-100 flex items-center flex-shrink-0">
             ...
             {doctor.imageUrl ? (<Image src={doctor.imageUrl} alt={doctor.name} width={48} height={48} className="rounded-full w-12 h-12 md:w-16 md:h-16 object-cover mr-3 md:mr-4 shadow-md"/>) : (<div className="rounded-full w-12 h-12 md:w-16 md:h-16 bg-gray-200 mr-3 md:mr-4"></div>)}
             <div>
               <h2 className="font-bold text-md md:text-lg text-gray-900">{doctor.name}</h2> {/* Adjusted size */}
               <p className="text-xs md:text-sm text-cyan-600">{doctor.specialty}</p> {/* Adjusted size */}
          ...
          <div ref={chatContainerRef} className="space-y-3 md:space-y-4 px-3 md:px-4 pb-4 flex-grow overflow-y-auto">
            {messages.map((message) => (
             <div key={message.id} className={`flex ${message.sender === 'patient' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] p-2 md:p-3 rounded-xl ${ message.sender === 'patient' ? 'bg-cyan-500 text-white rounded-br-none shadow-md' : 'bg-gray-100 text-gray-800 rounded-tl-none shadow-sm' }`}>
                  <p className="text-sm">{message.text}</p> {/* Ensure consistent text size */}
                  <p className={`text-xs mt-1 ${message.sender === 'patient' ? 'text-cyan-100 text-right' : 'text-gray-500 text-right'}`}>
                    {message.time}
                  </p>
                </div>
              </div>
